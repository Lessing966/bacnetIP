...
public class Ceshi {
    ...
    private static final Integer REMOTE_DEVICE_ID = 3965067;
    ...
    public static void main(String[] args) throws Exception {
        try {
            ...
            IpNetwork ipNetwork = new IpNetworkBuilder()
                    //本机的ip
                    .withLocalBindAddress("192.168.2.26")
            ...
            //analog-value 同上，只是对象属性标识符列表不同List
            PropertyValues byPropertyIdentifier = readValueByPropertyIdentifier(localDevice, remoteDevice, bvList, null, PropertyIdentifier.presentValue);
            PropertyValues byPropertyIdentifier1 = readValueByPropertyIdentifier(localDevice, remoteDevice, bvList, null, PropertyIdentifier.description);


            for (ObjectIdentifier oi : avList){
                System.out.println(oi.getObjectType().toString() + " " + oi.getInstanceNumber() + " PresentValue: " + byPropertyIdentifier.get(oi, PropertyIdentifier.presentValue).toString());
                System.out.println(oi.getObjectType().toString() + " " + oi.getInstanceNumber() + " Description: " + byPropertyIdentifier1.get(oi, PropertyIdentifier.description).toString());
            }
